<p align="center">
  <picture>
    ...
    <img src="./icon-light.png" alt="Activity" width="144" />
  </picture>
</p>
...
Everything happening on this node (monitor alerts, finished tasks, runs, approvals, meetings, notes), newest first, grouped by day.
...
**App:** [Install](ryu://apps/@ryu/activity) (opens the Ryu desktop app and asks you to confirm)

**CLI:**

```bash
ryu apps add @ryu/activity
```
...
- **`ui/` — companion (companion-only app, no backend crate).** A sandboxed
  ...
  `openActivitySession`), with no direct `fetch` and no node token in the sandbox, and
...
- **Capability grants:** `activity:read` for the feed/audit/trace reads and
  `activity:run` for provider-backed quality/security checks, online scoring,
  trace capture, and local audit retention maintenance.
  The app never executes a real tool from a security probe.
...
<a href="https://github.com/amajorai/ryu-activity/stargazers">
  <picture>
