...
import type { ActivityScoreInput, RyuBridge } from "./ryu.d.ts";
...
export function listObservabilityAudit(
	options: ActivityAuditOptions = {}
): Promise<unknown> {
	return ryu().activity.audit(options);
}

export function pruneObservabilityAudit(): Promise<unknown> {
	return ryu().activity.prune();
}

export function scoreObservabilityOutput(
	input: ActivityScoreInput
): Promise<unknown> {
	return ryu().activity.score(input);
}

export function getObservabilityTrace(runId: string): Promise<unknown> {
	return ryu().activity.trace({ run_id: runId });
}

export function runObservabilityRedteam(
	agentId: string,
	model?: string
): Promise<unknown> {
	return ryu().activity.redteam({ agent_id: agentId, model });
}

export function runObservabilityEval(
	agentId: string,
	model?: string
): Promise<unknown> {
	return ryu().activity.eval({ agent_id: agentId, model });
}

export function importObservabilityTrace(input: {
	agent_id?: string;
	run_id: string;
	suite_id?: string;
}): Promise<unknown> {
	return ryu().activity.importTrace(input);
}
