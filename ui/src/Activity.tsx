...
function dayLabel(createdAtSeconds: number): string {
	const timestamp = createdAtSeconds * MS_PER_SECOND;
...
export function Activity() {
	const [view, setView] = useState<"activity" | "observability">("activity");
	...
	return (
		<div className="h-full min-h-0 overflow-auto">
			<div
				className={`mx-auto flex w-full flex-col gap-6 p-6 ${view === "activity" ? "max-w-2xl" : "max-w-5xl"}`}
			>
				<header>
					<div className="flex flex-wrap items-start justify-between gap-3">
						<div>
							<h1 className="font-medium text-xl">
								{view === "activity" ? "Activity" : "Agent observability"}
							</h1>
							<p className="text-muted-foreground text-sm">
								{view === "activity"
									? "Everything happening on this node, newest first."
									: "Trace production behavior, evaluate quality, and probe safety on this node."}
							</p>
						</div>
						<div className="flex rounded-lg border p-0.5" role="tablist">
							{(
								...
								<Button
									...
									className="h-7 px-2.5 text-foreground text-xs"
									key={value}
			...
			{view === "observability" ? (
				...
				<PullToRefresh className="h-full" onRefresh={() => listQuery.refetch()}>
					<div className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-6 pb-6">
						{loading ? (
						...
						{!(loading || error) && items.length === 0 ? (
							<Empty className="py-10">
								<EmptyHeader>
									...
									<EmptyDescription>
										Monitor alerts, finished tasks, runs, and notes from this
										node will show up here as they happen.
									</EmptyDescription>
								</EmptyHeader>
							</Empty>
						) : null}
